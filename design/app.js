$(document).ready(function () {
		...
		function showError(error) {
			switch (error.code) {
				case error.PERMISSION_DENIED:
					showStatus(
						"error",
						'ERROR: Geolocation request denied. Try visiting the HTTPS site: <a href="https://codepen.io/tiffanyadu/pen/qryXBo" target="_blank">https://codepen.io/tiffanyadu/pen/qryXBo</a>'
					);
					break;
	...
	function processData(data) {
		...
		currentDate =
			daily[0].date.weekday +
			", " +
			daily[0].date.monthname +
			" " +
			daily[0].date.day +
			", " +
			daily[0].date.year;
		currentLocation =
			current.display_location.city + ", " + current.display_location.state;
		currentTemp = {
			c: current.temp_c,
			f: current.temp_f,
		};
	...
	function displayWeather() {
		...
		$forecastDivs.each(function (index) {
			...
			$(this)
				.find(".weatherIcon")
				.children()
				.attr("class", forecast[index].icon);
	...
	window.onload = function () {
		...
		setTimeout(function () {
			showStatus(
				"",
				'Click on the <i class="fa fa-location-arrow" aria-hidden="true"></i> button to share your current location.'
			);
